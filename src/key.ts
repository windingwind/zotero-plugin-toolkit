...
export class ZoteroKeyTool implements RegisterToolBase {
  ...
  private globalCache: ToolkitKeysGlobal;
   ...
   * @returns `true` for success and `false` for failure.
   */
  async unregisterKey(keyOptions: Key) {
    switch (keyOptions.type) {
      case "element":
        ((keyOptions as ElementKey).xulData.document || getGlobal("document"))
          .querySelector(`#${keyOptions.id}`)
          ?.remove();
        return true;
      case "prefs":
        getGlobal("Zotero").Prefs.set((keyOptions as PrefKey).id, "");
        return true;
      case "builtin":
        return false;
      case "event":
  ...
  unregisterAll(): void {
    ...
    this.globalCache.eventKeys
      ...
      .forEach((keyOptions) => this.unregisterKey(keyOptions));
  }
  ...
  private registerElementCommandset(commandSetOptions: ElementCommandSet) {
    commandSetOptions.document.querySelector("window").appendChild(
      this.uiTool.creatElementsFromJSON(commandSetOptions.document, {
        tag: "commandset",
        namespace: "xul",
  ...
  private registerElementCommand(commandOptions: ElementCommand) {
    ...
    commandOptions.document
      ...
      ?.appendChild(
        this.uiTool.creatElementsFromJSON(commandOptions.document, {
          tag: "command",
          namespace: "xul",
  ...
  private registerElementKeyset(keySetOptions: ElementKeySet) {
    keySetOptions.document.querySelector("window").appendChild(
      this.uiTool.creatElementsFromJSON(keySetOptions.document, {
        tag: "keyset",
        namespace: "xul",
        id: keySetOptions.id,
        skipIfExists: true,
  ...
  private registerElementKey(keyOptions: ElementKey) {
    const doc = keyOptions.xulData.document || getGlobal("document");
            ...
            (keyElem: XUL.Element) => {
              ...
              const key = {
                ...
                callback: (keyOptions: ElementKey) => {
                  ...
                  const _eval = getGlobal("eval").bind(win);
                  _eval(oncommand);
  ...
  private getMainWindowElementKeys() {
    return this.getElementKeys(getGlobal("document"));
  }
